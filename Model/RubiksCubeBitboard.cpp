...
class RubiksCubeBitboard : public RubiksCube
{
    ...
    void rotateSide(const int s1, const int s1_1, const int s1_2, const int s1_3, const int s2, const int s2_1,
                    const int s2_2, const int s2_3)
    {
...
public:
    ...
    {
        const int idx = arr[row][col];
        if (idx == 8)
        {
            return static_cast<COLOR>(static_cast<int>(face));
        }
        const uint64_t side = bitboard[static_cast<int>(face)];
    ...
    {
        this->rotateFace(1);
        const uint64_t clr1 = (bitboard[2] & (one_8 << (8 * 0))) >> (8 * 0);
        const uint64_t clr2 = (bitboard[2] & (one_8 << (8 * 6))) >> (8 * 6);
        const uint64_t clr3 = (bitboard[2] & (one_8 << (8 * 7))) >> (8 * 7);
    ...
    RubiksCube& F() override
    {
        ...
        const uint64_t clr1 = (bitboard[0] & (one_8 << (8 * 4))) >> (8 * 4);
        const uint64_t clr2 = (bitboard[0] & (one_8 << (8 * 5))) >> (8 * 5);
        const uint64_t clr3 = (bitboard[0] & (one_8 << (8 * 6))) >> (8 * 6);
    ...
    {
        this->rotateFace(3);
        const uint64_t clr1 = (bitboard[0] & (one_8 << (8 * 2))) >> (8 * 2);
        const uint64_t clr2 = (bitboard[0] & (one_8 << (8 * 3))) >> (8 * 3);
        const uint64_t clr3 = (bitboard[0] & (one_8 << (8 * 4))) >> (8 * 4);
    ...
    RubiksCube& B() override
    {
        ...
        const uint64_t clr1 = (bitboard[0] & (one_8 << (8 * 0))) >> (8 * 0);
        const uint64_t clr2 = (bitboard[0] & (one_8 << (8 * 1))) >> (8 * 1);
        const uint64_t clr3 = (bitboard[0] & (one_8 << (8 * 2))) >> (8 * 2);
    ...
    RubiksCube& D() override
    {
        ...
        const uint64_t clr1 = (bitboard[2] & (one_8 << (8 * 4))) >> (8 * 4);
        const uint64_t clr2 = (bitboard[2] & (one_8 << (8 * 5))) >> (8 * 5);
        const uint64_t clr3 = (bitboard[2] & (one_8 << (8 * 6))) >> (8 * 6);
    ...
    [[nodiscard]] uint64_t getCorners() const
    {
        uint64_t ret = 0;
